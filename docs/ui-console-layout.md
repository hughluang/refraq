...
Paged **Offset Page** tables render through `ListTable`. The primitive owns one visual contract:
...
3. **Empty, no-match, and error are in-table status rows.** The empty gate reads `total`, not `items.length`. A vacant filter result is a no-match state, distinct from a vacant collection. A fetch error with no rows to keep is an in-table error row with retry; a fetch error that still has rows keeps the table. `usePagedList` Offset Page callers pass `onError` so that later failure toasts; Refine `useTable` lists use the default list-error notification. Default copy is `common.empty` / `common.noMatch`; a page may override either string.
...
5. **Parents own available block size; `ListTable` owns scrolling and pager placement.** Console surfaces are a few stable frames: AppShell header plus a main frame that receives remaining viewport space; `PageChrome` keeps breadcrumb/title/toolbar at intrinsic height and passes leftover space to content; `ListTable` is a flex column whose `Table.ScrollContainer` consumes that leftover and whose pager stays outside the scroll region. Remaining-space frames that wrap a list in `FillColumn` (`frontend/src/components/layout/FillColumn.tsx`) so the table fills the region after intrinsic siblings are: full-page Offset Page lists, the catalog Joins tab, and the schedule-jobs modal body (already a remaining-space column; it does not wrap again). Horizontal `minWidth` is independent of this vertical contract; the schedule-jobs modal passes a narrower value.

**Form surfaces vs remaining-space frames.** Account Center and Platform Settings are form pages: intrinsic sections stack under `PageChrome`, and `PageChrome` `overflow: auto` is the page scroller when content exceeds the viewport. User PAT inside Account Center stays an Offset Page table (`ListTable` + `ListPager`) but is a section of that form page — it does not consume leftover viewport height and must not wrap the whole Account page (or the PAT section alone) in `FillColumn`. Do not judge Account success by “pager pinned in the first viewport.”

List-state decisions live in `frontend/src/lib/list-state.ts` (`listPresentationOf` returns `{ state, refreshing }`). Pages keep writing their own header and body cells; `ListTable` is not a column DSL. Callers keep API-error message normalization in the feature.

**Offset Page callers** (must use `ListTable`): Catalog Browse, Users, Roles, Sources, Jobs, Schedules, Business Domains, Type Mappings, User PAT (Account Center), Source-scoped schedules, Structure Diff list, catalog Joins tab, schedule-jobs modal.

**Not Offset Page collection lists** (do not migrate to `ListTable`): Catalog Columns (client-side filter and slice; may compose `ListPager` directly), Catalog Sample (row peek), Structure Diff detail (static tables).
...
- On form pages, `PageChrome` may scroll; that is expected. On remaining-space list frames and the Joins tab, the table scroller owns vertical overflow and `PageChrome` should not become a second scroller.
